...
public class ProductionRiskAssessmentService : IProductionRiskAssessmentService
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public ProductionRiskAssessmentService(IUnitOfWork unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }

    public async Task<RiskAssessmentListDto> CreateAsync(CreateProductionRiskAssessmentDto createDto, string keycloakSub)
    {
        ...
        var existingAssessment = await _unitOfWork.ProductionRiskAssessments
        ...
        var entity = _mapper.Map<ProductionRiskAssessment>(createDto);
        entity.UserId = user.Id;
        entity.CreatedAt = DateTime.UtcNow;

        await _unitOfWork.ProductionRiskAssessments.AddAsync(entity);
        await _unitOfWork.SaveChangesAsync();

        var result = _mapper.Map<RiskAssessmentListDto>(entity);
        result.AssessmentType = "Production";
        return result;
    }

    public async Task<RiskAssessmentListDto?> GetByIdAsync(int id)
    {
        var entity = await _unitOfWork.ProductionRiskAssessments.GetByIdAsync(id);
    ...
    {
        var entities = await _unitOfWork.ProductionRiskAssessments
            .FindAsync(x => x.MaintenanceRequestId == maintenanceRequestId);

        var results = _mapper.Map<List<RiskAssessmentListDto>>(entities);
        results.ForEach(x => x.AssessmentType = "Production");
        return results;
    }

    public async Task<RiskAssessmentListDto?> UpdateAsync(int id, CreateProductionRiskAssessmentDto updateDto, string keycloakSub)
    {
        if (string.IsNullOrWhiteSpace(keycloakSub))
            throw new ArgumentException("Keycloak Subject is required", nameof(keycloakSub));
        ...
        _unitOfWork.ProductionRiskAssessments.Update(entity);
        await _unitOfWork.SaveChangesAsync();
