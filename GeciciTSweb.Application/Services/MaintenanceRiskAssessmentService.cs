...
public class MaintenanceRiskAssessmentService : IMaintenanceRiskAssessmentService
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IMapper _mapper;

    public MaintenanceRiskAssessmentService(IUnitOfWork unitOfWork, IMapper mapper)
    {
        _unitOfWork = unitOfWork;
        _mapper = mapper;
    }

    public async Task<RiskAssessmentListDto> CreateAsync(CreateMaintenanceRiskAssessmentDto createDto, string keycloakSub)
    {
        ...
        var existingAssessment = await _unitOfWork.MaintenanceRiskAssessments
        ...
        var entity = _mapper.Map<MaintenanceRiskAssessment>(createDto);
        entity.UserId = user.Id;
        entity.CreatedAt = DateTime.UtcNow;

        await _unitOfWork.MaintenanceRiskAssessments.AddAsync(entity);
        await _unitOfWork.SaveChangesAsync();

        var result = _mapper.Map<RiskAssessmentListDto>(entity);
        result.AssessmentType = "Maintenance";
        return result;
    }

    public async Task<RiskAssessmentListDto?> GetByIdAsync(int id)
    {
        var entity = await _unitOfWork.MaintenanceRiskAssessments.GetByIdAsync(id);
    ...
    {
        var entities = await _unitOfWork.MaintenanceRiskAssessments
            .FindAsync(x => x.MaintenanceRequestId == maintenanceRequestId);

        var results = _mapper.Map<List<RiskAssessmentListDto>>(entities);
        results.ForEach(x => x.AssessmentType = "Maintenance");
        return results;
    }

    public async Task<RiskAssessmentListDto?> UpdateAsync(int id, CreateMaintenanceRiskAssessmentDto updateDto, string keycloakSub)
    {
        if (string.IsNullOrWhiteSpace(keycloakSub))
            throw new ArgumentException("Keycloak Subject is required", nameof(keycloakSub));
        ...
        _unitOfWork.MaintenanceRiskAssessments.Update(entity);
        await _unitOfWork.SaveChangesAsync();
